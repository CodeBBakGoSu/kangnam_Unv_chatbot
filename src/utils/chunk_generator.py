...
def extract_chunks_from_course(course: dict) -> list:
    """
...
def generate_all_chunks(input_file_path: str, output_file_path: str):
    """
    입력 JSON 파일에서 모든 과목 데이터를 읽어 청크를 생성하고 출력 JSON 파일로 저장합니다.
    """
    all_chunks = []
    try:
        ...
        courses = processed_data.get("courses", [])
        ...
        print(f"청크 생성 중 알 수 없는 오류 발생: {e}")
        return

    try:
        output_dir = os.path.dirname(output_file_path)
        if output_dir and not os.path.exists(output_dir): # output_dir이 존재하고, 해당 경로가 실제로 없는 경우
            os.makedirs(output_dir)
            print(f"출력 디렉토리 '{output_dir}'을(를) 생성했습니다.")

        with open(output_file_path, 'w', encoding='utf-8') as f:
            json.dump(all_chunks, f, indent=2, ensure_ascii=False)
        print(f"생성된 청크 데이터가 '{output_file_path}' 파일로 저장되었습니다. 총 {len(all_chunks)}개의 청크가 생성되었습니다.")
    except Exception as e:
        print(f"청크 데이터를 파일로 저장하는 중 오류 발생: {e}")

if __name__ == "__main__":
    # 이 스크립트(chunk_generator.py)는 src/utils/ 내에 위치한다고 가정
    current_script_dir = os.path.dirname(os.path.abspath(__file__)) # src/utils
    src_dir = os.path.dirname(current_script_dir) # src
    workspace_root = os.path.dirname(src_dir) # workspace_root (cap 폴더)

    # 입력 및 출력 파일 경로 설정 (user_data 폴더 내부)
    input_json_path = os.path.join(workspace_root, "user_data", "kangnam_courses_processed.json")
    output_json_path = os.path.join(workspace_root, "user_data", "course_chunks.json")
    
    print(f"입력 파일 경로: {input_json_path}")
    print(f"출력 파일 경로: {output_json_path}")

    generate_all_chunks(input_json_path, output_json_path)

    # # 생성된 청크 중 처음 5개만 출력해서 확인 (필요시 주석 해제)
    # try:
    ...
    #             print("생성된 청크가 없습니다.")
    # except FileNotFoundError:
    #     print(f"오류: 청크 결과 파일 '{output_json_path}'을(를) 찾을 수 없습니다.")
    # except Exception as e:
    #     print(f"생성된 청크 파일 확인 중 오류: {e}")
