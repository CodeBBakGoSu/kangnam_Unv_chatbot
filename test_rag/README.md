...
```bash
uv run python test_rag/test_search_tools.py
```
...
```bash
uv run python test_rag/data/test_corpus_query.py
```
...
from test_rag.tools.search_tools import ALL_SEARCH_TOOLS
...
```bash
uv run python test_rag/data/upload_to_rag.py
```
...
```bash
uv run python test_rag/data/delete_corpus_files.py
```
